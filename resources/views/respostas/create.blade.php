                        ...
                        <div class="question_{{ $q }}">
                            ...
                            @if ($question->tipo === 'Escolha Única')
                                ...
                                @foreach ($question->options as $option)
                                    ...
                                    <div class="form-check">
                                        <input type="radio" name="option_id[{{ $question->id }}]"
                                            value="{{ $option->id }}" />
                            ...
                            @elseif ($question->tipo === 'Escolha Múltipla')
                                ...
                                @foreach ($question->options as $option)
                                    ...
                                    {{-- Bloco Radio buttons --}}
                                    <div class="form-check">
                                        <input type="checkbox" name="option_id[{{ $question->id }}][]"
                                            value="{{ $option->id }}" />
                                        <label class="form-check-label" for="{{ $question->id . $o }}">
                                            {{ $option->titulo }}
                                        </label>
                            ...
                            @else
                                <div class="form-floating">
                                    <textarea class="form-control" name="comment[{{ $question->id }}][]" style="height: 100px"></textarea>
