        ...
        <div class="col-md-12">
            <div class="card">
                <div class="car-header">
                    <h4 class="text-center mt-2">Dados dos usuários</h4>
                    <a href="{{ route('user.create') }}" class="btn btn-primary float-end btn-sm mb-2 mr-3 add_user">Cadastrar</a>
                </div>
                <div class="card-body">
                    <x-adminlte-datatable id="consulta" class="consulta_table" :heads="$heads" :config="$config" striped hoverable bordered compressed />
                </div>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function($) {
        ...
        $('body').on('click', '.btnEdit', function() {
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Botão editar foi clicado!',
        ...
        $('body').on('click', '.btnDelete', function() {
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Botão delete foi clicado!',
                showConfirmButton: false,
                timer: 1500
            })
        });
        ...
        $('body').on('click', '.btnDetails', function() {
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Botão details foi clicado!',
                showConfirmButton: false,
                timer: 1500
            })
        });
